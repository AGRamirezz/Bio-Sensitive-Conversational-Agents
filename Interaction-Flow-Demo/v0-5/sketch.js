...
function setupWebSocket() {
  ...
  socket.onopen = () => console.log("✅ Connected to EEG WebSocket");

  socket.onmessage = (event) => {
    try {
      const data = JSON.parse(event.data);
      if (data.channel === "AF7") {
        updateBioSignalsFromLive(data);
      }
    } catch (err) {
  ...
  socket.onclose = () => {
    console.warn("⚠️ EEG WebSocket closed");
...
function updateBioSignalsFromLive(eegData) {
  console.log("📥 live EEG:", eegData);

  // Scale raw band powers into 0-1 range - adjust SCALE experimentally
  const SCALE = 1e-5;
  alphaAmplitude = eegData.alpha * SCALE;
  betaAmplitude  = eegData.beta  * SCALE;
  thetaAmplitude = eegData.theta * SCALE;
  deltaAmplitude = eegData.delta * SCALE;
